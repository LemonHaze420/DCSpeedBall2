...
/*()									\
{														\
	DFH = _wfopen(TEXT("\\SBDebug.txt"), TEXT("wt"));	\
}*/
...
#define	DEBUG_LOCALE(a)			;
/*									\
{																	\
	char	buffer[256];											\
	sprintf(buffer, "Reached %s:%d " #a "\n", __FILE__, __LINE__);	\
	fwrite(buffer, strlen(buffer) + 1, 1, DFH);						\
}*/


#define	DEBUG_VALUE(a)			;
/*									\
{																	\
	char	buffer[256];											\
	sprintf(buffer, #a " = %d\n", (a));								\
...
{																									\
	TCHAR dbmsg[256];																				\
	swprintf(dbmsg, L"WARNING: Null object used at line \"%d\" in \"%hs\"\n", __LINE__,  __FILE__);	\
